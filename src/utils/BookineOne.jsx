...
const BookingOne = () => {
  ...
  async function handleClick() {
    ...
    try {
      const res = await axios.post(`${BASE_URL}GenerateOtp`, { email: email.trim() }, { withCredentials: true });
      console.log(res);
  ...
  return (
    <div className="bgImageforVerificationOfEmailId relative w-full h-screen pt-[100px]">
      ...
      {!isOnBookingDetails ? (
        ...
        <div className="absolute top-[11%] left-[37%] z-20 flex justify-center items-center px-4">
          <div className="w-full max-w-sm md:max-w-md bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl shadow-2xl text-white p-5 sm:p-7">
            ...
            <div className="mb-4">
              <input
                ...
                disabled={isLoading}
                className={`w-full h-[45px] px-4 text-sm text-gray-900 bg-white rounded-lg shadow-sm border transition-all duration-200 focus:outline-none focus:ring-2 focus:border-transparent opacity-80 disabled:opacity-50 disabled:cursor-not-allowed ${
                  error 
            ...
            <button
              ...
              className={`w-full py-2 font-medium rounded-lg transition-all duration-300 shadow-md hover:shadow-lg ${
